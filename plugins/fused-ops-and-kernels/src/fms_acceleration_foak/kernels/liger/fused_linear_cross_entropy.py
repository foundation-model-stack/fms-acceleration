...
from ..unsloth.cross_entropy_loss import _chunked_cross_entropy_forward, _cross_entropy_backward
...
def fused_linear_cross_entropy_forward(
    ...
    # TODO _input is a dummy tensor
    # need to retrieve _input from previously stored value
    ...
    for chunk_id in range(num_chunks):
        ...
        logsumexp = torch.empty((n_rows, num_chunks,), dtype = torch.float32, device = "cuda")
        _chunked_cross_entropy_forward[(n_rows, num_chunks,)](
            logits_chunk, 
...
def fused_linear_cross_entropy_backward(
    grad_output, grad_input, grad_weight, grad_bias, target
):
    ...
    if torch.ne(grad_output, torch.tensor(1.0, device=grad_output.device)):
        ...
        # grad_output is dlosses
        n_rows, vocab_size = grad_input.shape
        div, mod = divmod(vocab_size, BLOCK_SIZE)
        n_blocks = div + (mod != 0)
        logsumexp = torch.empty((n_rows, n_blocks,), dtype = torch.float32, device = "cuda")
        _cross_entropy_backward[(n_rows, n_blocks,)](
            grad_input,
            grad_input.stride(0),
            grad_output, 
...
class LigerFusedLinearCrossEntropyFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        (grad_input, grad_weight, grad_bias, target) = ctx.saved_tensors
